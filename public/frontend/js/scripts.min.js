function setActiveColor(t) {
    for (var e, i = 0; e = document.getElementsByTagName("link")[i]; i++) - 1 != e.getAttribute("rel").indexOf("style") && e.getAttribute("title") && (e.disabled = !0, e.getAttribute("title") == t && (e.disabled = !1))
}

function getActiveColor() {
    for (var t, e = 0; t = document.getElementsByTagName("link")[e]; e++)
...
function generateCookie(t, e, i) {
    var n = i ? ((n = new Date).setTime(n.getTime() + 24 * i * 60 * 60 * 1e3), "; expires=" + n.toGMTString()) : "";
...
document.addEventListener("DOMContentLoaded", function () {
    "use strict";
    loadWindowSettings(), loadWindowEvents(), loadMenu(), loadTabs(), izotope(), popup(), accordion(), loadTooltips(), initSliderUI(), loadCountdown(), speacialCount(), loadSkills(), misc(), slick(), listenSlick(), loader(), backtotop(), owldemo(), isotope(), livechat(), contactform(), popover(), scrollgoto(), active(), display(), switching(), translate(), switchVisible(), setActiveColor()
}), window.onload = function (t) {
    setActiveColor(goCookie("style") || getSetColor())
...
function translate() {
    $.getScript("frontend/js/lib/i18next.min.js", function () {
        $.getScript("frontend/js/lib/i18nextXHRBackend.min.js", function () {
            $.getScript("frontend/js/lib/jquery-i18next.min.js", function () {
                var t = localStorage.getItem("lng");
                t || (localStorage.setItem("lng", "en-US"), t = "en-US"), i18next.use(i18nextXHRBackend).init({
                    lng: t,
                    fallbackLng: "en-US",
                    backend: {
    ...
    $(window).on("load", function () {
        ...
        t.on("click", function () {
            if ($(".mo", this).toggleClass("active"), $(".an", this).toggleClass("active"), $(".month", this).toggleClass("active"), $(".switch", this).toggleClass("on"), $(".year", this).toggleClass("active"), e.hasClass("annually")) {
                e.text("month");
                for (var t = 0; t <= 4; t++) $(".price-container:eq(" + t + ") .value").text(parseFloat(Number($(".price-container:eq(" + t + ") .value").text()) / 12).toFixed(2)), $(".price-container:eq(" + t + ") .discount").text(parseFloat(Number($(".price-container:eq(" + t + ") .discount").text()) / 12).toFixed(2))
            } else
                for (e.text("year"), t = 0; t <= 4; t++) $(".price-container:eq(" + t + ") .value").text(parseFloat(12 * Number($(".price-container:eq(" + t + ") .value").text())).toFixed(2)), $(".price-container:eq(" + t + ") .discount").text(parseFloat(12 * Number($(".price-container:eq(" + t + ") .discount").text())).toFixed(2));
...
setActiveColor(title), $("img.svg").each(function () {
        var i = jQuery(this),
            n = i.prop("attributes"),
            t = i.attr("src");
        $.get(t, function (t) {
            var e = (e = $(t).find("svg")).removeAttr("xmlns:a");
            $.each(n, function () {
    ...
    function (t) {
        t(document).ready(function () {
            t(".styleswitch").click(function () {
                ...
                return t("link[rel*=style][title]").each(function (t) {
...
function initSliderUI() {
    ...
    $(".slider-ui").length && $(".slider-ui").each(function () {
        var t = $(this),
            ...
            n = "sliderUI" + c,
            o = "inputUI" + c,
            t = parseInt(i.attr("data-start")),
            a = parseInt(i.attr("data-count-step"));
        e.attr("id", n), i.attr("id", o), c++, a = a || 300;
        ...
        }), s.noUiSlider.on("change", function (t, e, i, n) {
            $(this.target).closest(".sidebar").find(".circle").attr("data-percent", parseInt(i) / a * 100)
        }), s.noUiSlider.on("update", function (t, e) {
            var i = t[e],
                n = r();
            $("#disk-val").html(n[i].disk), $("#cpu-val").html(n[i].cpu), $("#ram-val").html(n[i].ram), $("#setup-val").html(n[i].setup), $("#ip-val").html(n[i].ip), $("#bw-val").html(n[i].bandwith), $("#price-val").html(n[i].price), $("#priceon-val").html(n[i].priceon), $("#orderlink-val").html(n[i].orderlink), l.value = t[e], t = t[e], e = r(), $("#orderlink").attr("href", e[t].orderlink)
        }), l.addEventListener("change", function () {
            s.noUiSlider.set([null, this.value])
        }), l.addEventListener("keydown", function (t) {
            var e = Number(s.noUiSlider.get()),
                i = (i = s.noUiSlider.steps())[0];
            switch (t.which) {
                case 13:
                    s.noUiSlider.set(this.value);
                    break;
                case 38:
                    s.noUiSlider.set(e + i[1]);
                    break;
                case 40:
                    s.noUiSlider.set(e - i[0])
            }
        })
    })
}
...
function izotope() {
    var e;
    $(".izotope-container").length && (e = $(".izotope-container")).isotope({
        itemSelector: ".item",
        layoutMode: "masonry",
        masonry: {
            columnWidth: ".item"
...
function offheight() {
    var t;
    750 < $(window).width() && (t = $(".offers").outerHeight(!0), $(".offers.light").css("height", t + 1))
}
...
function updateSlidesPerView(t, e, i, n) {
    var o = $(window).width();
    return $(window).height(), 1199 < o ? n : 991 < o ? i : 700 < o ? e : t
}
$(function () {
    $('[data-bs-toggle="tooltip"]').tooltip()
});
