	...
	<div>
			...
			<table>
				<tr>
					...
					<td class="text-right invoice">
						<span >INVOICE</span> <br> <span># {{ $bill->student->id }}</span>
					</td>
				</tr>
		...
		<div style="padding: 1.5rem;padding-bottom: 0">
            <table>
				...
				<tr><td class="strong"><b>{{ $bill->student->student_name }}</b></td></tr>

				<tr><td class="small bold text-right">{{ __('Date') }}: {{ date('d-M-Y') }}</td></tr>
				<tr><td class="small bold text-right">{{ __('Payment Terms') }}: Month Payment</td></tr>
				@if($bill->student->email !== null )
				<tr><td class="small bold text-right">{{ __('Email') }}: {{ $bill->student->email }}</tr>
				@endif
				<tr><td class="small bold text-right">{{ __('Phone') }}: {{ $bill->student->phone }}</td></tr>
				@if($bill->student->due_amount !== null)
				<tr><td class="small due-amount bold text-right">{{ __('Due Amount') }}: Tk {{ $bill->due }}</td></tr>
				@endif
			</table>
		</div>
			...
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr style="background: #e3e7f0;color:red !important;">
	                    <th width="35%">{{ __('Month') }}</th>
	                    <th width="10%">{{ __('Discount Amount') }}</th>
